...
public class Bank {
    ...
    public void openAccount(String accountid) throws AccountExistsException {
        if (accountlist.containsKey(accountid)) {
            ...
            //changed line 45, it was only accountid given in the put() method, now it is correct
        }
    }
    ...
    public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
        if (!accountlist.containsKey(accountid)) {
            ...
            Account account = accountlist.get(accountid);
            account.withdraw(money);//was deposit(money) before, had to change it to withdraw(money)
        }
    }
	...
	public Integer getBalance(String accountid) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			//in here note that we have to change the getter method from the money to make it type Integer, not Double
		}
	}
